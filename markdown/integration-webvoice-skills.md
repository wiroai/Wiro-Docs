...
Issues two opaque, one-time handles, both valid until `expiresAt` (5 minutes after issue): a `sessionToken` the browser presents to the WebSocket on first connect, and a `cancelToken` for `Realtime/Cancel`. No API key ever needs to reach the browser.
...
|-----------|------|----------|-------------|
| `useragentguid` | string | Yes | Useragent instance guid. Must be an agent you deployed (for a team agent, call with that team's project key or workspace, or remain an active member of the team), must be in `status: 4` (Running), and must have `util-web-channel` enabled. |
...
| Field | Description |
|-------|-------------|
| `correlationId` | Random public identifier for this session. Use it as the audio frame prefix (Step 2); server audio frames and the `Realtime/Cancel` response carry it too. |
...
| `sessionToken` | Opaque one-time handle, valid until `expiresAt`; don't parse it. Send it as the **first** WebSocket message (`{ type: "session_start", sessionToken }`); it works only once. |
| `cancelToken` | Opaque one-time handle for `Realtime/Cancel` (Step 3), valid until `expiresAt`. Only the user who called `WebStart` can use it. |
| `expiresAt` | Expiry of both handles, in ms since epoch (5 minutes after issue). After this point, call `WebStart` again for fresh handles. |
...
Open a WebSocket to the returned `wsUrl` and send the `session_start` JSON frame **within 10 seconds**. After that, frame microphone audio as 24 kHz mono PCM int16 with a `<correlationId>|` text prefix. **Server-sent binary audio frames use the same prefix shape — you must strip it before feeding the bytes to Web Audio.**
...
//    correlationId / wsUrl / sessionToken / cancelToken down to the page.
//    Never expose your Wiro API key to the page.
const { correlationId, wsUrl, sessionToken, cancelToken } = await fetch("/api/voice/start").then((r) => r.json());
...
ws.addEventListener("message", async (event) => {
  if (event.data instanceof ArrayBuffer) {
    // Binary frame: <correlationId>|<pcm-int16-24khz-mono>. STRIP the prefix.
...
//    that we frame as `<correlationId>|<binary>` and ship over the socket.
...
async function startMicCapture() {
  ...
  const sessionMarker = new TextEncoder().encode(correlationId);
...
If the browser can't progress past `WebStart` (mic permission denied, user navigated away, tab crashed), call `Realtime/Cancel` so the server-side agent prep tears down immediately instead of waiting on the 5-minute cleanup guard. Send the `cancelToken` from `WebStart`, authenticated as the user who called `WebStart`; each cancel handle works once.
...
curl -X POST "https://api.wiro.ai/v1/UserAgent/Realtime/Cancel" \
  ...
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "cancelToken": "CANCEL_TOKEN_FROM_WEBSTART"
  }'
```
...
| HTTP | Error | When |
|------|-------|------|
| 400 | `cancelToken required` | `cancelToken` is missing. |
| 401 | `invalid token` | Unknown, already used or expired handle. |
| 403 | `access denied` | The handle was issued to another user. |

> No-op when the session is already cleaned up (bridge teardown or the 5-min cleanup guard). The endpoint always returns `cancelled: true` on success — there's no separate "already cleaned" signal. Reusing a cancel handle returns `invalid token`.
...
```json
{ "type": "session_start", "sessionToken": "<sessionToken-from-WebStart>" }
```

Must arrive within 10 seconds of the WebSocket `open` event. Late or malformed → the server closes the socket with no further messages. The handle works once and only until `expiresAt`; an unknown, used or expired handle closes the socket the same way. The URL carries no session identifier.
...
| Frame | `<correlationId>|<binary-pcm>` — UTF-8 text prefix (the `correlationId` returned by `WebStart`) + `|` (0x7C) separator + raw PCM bytes. Same shape as the [Realtime Voice Conversation](/docs/realtime-voice-conversation#audio-format) channel. **Both directions** use this prefix — when you receive a binary frame from the server you must locate the first `0x7C` byte and slice everything after it as `Int16Array`. |
...
| Binary frame | Agent audio chunk — `<correlationId>|<int16-pcm-24khz-mono>`. Strip the prefix and feed the PCM tail to Web Audio. |
...
- The `sessionToken` handle is valid for **300 seconds** from issue (until `expiresAt`) and works once, so the browser must send `session_start` before then. To connect again, call `WebStart` for fresh handles — there's no in-band refresh.
...
| 403  | `96`       | `useragent-access-denied`| Caller didn't deploy the useragent. |
| 200  | —          | `This agent belongs to a team. Switch to the team context to access it.` | Team agent called from a personal workspace or personal project key by a caller who is no longer a member of the agent's team. |
| 200  | —          | `This agent belongs to a different team.` | Team agent called from another team's workspace or project key by a caller who is no longer a member of the agent's team. |
...
- **One-time handles** — `sessionToken` and `cancelToken` are opaque, random and single-use, valid until `expiresAt`. Nothing readable travels in the handle or the WS URL — same "single endpoint, first-message token" pattern Wiro uses for its native realtime models.
- **`useragentguid` + caller uuid** — kept on the server behind the handle, never sent to the browser; the bridge re-checks the agent when the browser connects.
