...
> **Only user-writable fields are accepted.** `appid` and `appsecret` are user-writable by default in the `metaads` template. Attempts to set platform-managed fields are silently ignored. Call `POST /UserAgent/Detail` and inspect the `metaads` credential block if you see a silent no-op.
